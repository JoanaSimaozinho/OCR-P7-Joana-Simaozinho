...
describe(__filename + "#", () => {
    [
        ...
        it(`compiles ${input} -> ${output}`, () => __awaiter(void 0, void 0, void 0, function* () {
            let source = `<style>${input}</style>`;
            if (scoped) {
                source = `<div>${source}</div>`;
            }
            const engine = yield createMockEngine({
                "/entry.pc": source
            });
            const text = stringifyLoadResult(yield engine.open("/entry.pc")).match(/<style>(.*?)<\/style>/)[1];
