...
import { findPCConfigUrl, PC_CONFIG_FILE_NAME } from "paperclip-utils";
...
const findResourcesFromConfig = (getResources) => (fs) => (fromUri, relative) => {
...
const resolveModuleRoots = fs => (fromDir, roots = []) => {
...
const filterAllFiles = fs => (filter) => {
...
const resolveResources = (fs) => (config, cwd, filterFiles) => {
    ...
    if (config.moduleDirs) {
        for (const modulesDirname of config.moduleDirs) {
            ...
            for (const moduleDir of moduleRoots) {
                ...
                const moduleConfig = JSON.parse(fs.readFileSync(pcConfigPath, "utf8"));
                const moduleSources = filterFiles(path.join(moduleDir, moduleConfig.srcDir));
...
export const resolveAllPaperclipFiles = findResourcesFromConfig((fs) => (config, cwd) => {
    return resolveResources(fs)(config, cwd, filterAllFiles(fs)(filePath => path.extname(filePath) === ".pc" || path.extname(filePath) === ".css"));
});
export const resolveAllAssetFiles = findResourcesFromConfig((fs) => (config, cwd) => {
    // const ext = `+(jpg|jpeg|png|gif|svg)`;
    const exts = [".jpg", ".jpeg", ".png", ".gif", ".svg", ".ttf"];
    // const sourceDir = config.srcDir;
    return resolveResources(fs)(config, cwd, filterAllFiles(fs)(filePath => exts.includes(path.extname(filePath))));
    // if (sourceDir === ".") {
